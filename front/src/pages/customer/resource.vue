          ...
          if (isNaN(values.newseller)) {
              ...
              if (Object.prototype.toString.call(name) !== "[object Undefined]") {
                  for (let i = 0; i < employeeList.length; i++) {
                      let elementObj = employeeList[i];
                      for (let keys in elementObj) {
                          if (keys == 'name' && elementObj[keys] == name) {
                              newsellerId = elementObj.id
                          }
                      }
                  }
              }
              values.newseller = newsellerId
          }
